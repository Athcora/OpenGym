...
type TeamSubstituteRequest = { id:string;team_id:string;requester_id:string|null;target_id:string;status:string;team?:KingTeam };
type KingDropPlacement = { teamId:string; index:number };
...
function displayedTeamLabel(team:KingTeam,teams:KingTeam[],courtCount:number){
  if(team.status==='current'&&team.court_number!==null&&team.court_side!==null){
    return `Team ${2*(team.court_number-1)+team.court_side}`;
  }
  const waitingTeams=teams.filter(item=>item.status==='waiting').sort((a,b)=>a.queue_position-b.queue_position);
  const waitingIndex=waitingTeams.findIndex(item=>item.id===team.id);
  const waitingOrdinal=waitingIndex>=0?waitingIndex+1:Math.max(1,team.queue_position);
  return `Team ${2*courtCount+waitingOrdinal}`;
}
...
function preventNativeTouchScroll(event:TouchEvent){
  // Arm this on pointer-down so iOS keeps the gesture available to the app,
  // but block native scrolling only after the deliberate hold activates.
...
export default function App({initialFacilitySlug}:{initialFacilitySlug?:string}={}) {
  ...
  const [teamSubstituteRequests,setTeamSubstituteRequests]=useState<TeamSubstituteRequest[]>([]);
  ...
  const meHasActiveStatus=Boolean(activeMe&&['current','waiting','sitout'].includes(activeMe.status));
  const ownTeamSubstitute=activeMe?teamSubstitutes.find(substitute=>substitute.player_id===activeMe.id):undefined;
  const meIsVisibleInQueue=Boolean(meHasActiveStatus&&activeMe&&(!isTeamsMode(config.mode)||Boolean(ownTeamSubstitute)||kingTeams.some(team=>team.id===activeMe.team_id&&team.members.some(member=>member.id===activeMe.id))));
  ...
  useEffect(()=>{
    if(!user||!me||admin||host||onboarding!=='idle'||hostTutorial||hostAppointmentNotice||me.status==='left'||me.status==='rejoin')return;
    if(needsTutorial(user,config.mode))setOnboarding('disclaimer');
  },[user?.id,user?.user_metadata?.opengym_tutorial_version,me?.id,me?.status,admin,host,config.mode,onboarding,hostTutorial,hostAppointmentNotice]);
  ...
  async function boot(){
    ...
    const channel=supabase.channel(`live-waitlist:${selected.id}`)
      ...
      .on('postgres_changes',{event:'*',schema:'public',table:'team_fill_ins'},scheduleRefresh)
      ...
      .on('postgres_changes',{event:'*',schema:'public',table:'team_substitutes'},scheduleRefresh)
      .on('postgres_changes',{event:'*',schema:'public',table:'team_substitute_requests'},scheduleRefresh)
      ...
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'waitlist_events'},payload=>{
        ...
        if(event.event_type==='team_substitute_invite'){scheduleRefresh();return;}
        ...
        if((event.event_type==='team_rotation'||event.event_type==='king_game')&&event.actor_name&&event.actor_user_id!==session?.user.id){
          ...
          const shouldNotify=adminAccess.current||ownHostStatus.current||Boolean(player&&(
            player.status!=='current'||player.court_number===courtNumber
          ));
  ...
  async function refresh(activeUser?:User|null,expectedFacility=facilityRef.current,contextReady=false){
    ...
    const [{data:fillRows},{data:teamSubRows},{data:teamSubRequestRows}]=await Promise.all([
      ...
      supabase.from('team_substitute_requests').select('id,team_id,requester_id,target_id,status').eq('status','pending').gt('created_at',new Date(Date.now()-5*60_000).toISOString()),
    ]);
  ...
  async function completeTutorial(){
    ...
    const versionKey=tutorialVersionKey(config.mode);
    const {data,error}=await supabase.auth.updateUser({data:{opengym_tutorial_completed:true,[versionKey]:TUTORIAL_VERSION}});
  ...
  function ask(title:string,message:string,confirm:string,action:()=>Promise<void>,actionTone:'danger'|'success'='danger',cancelTone:'neutral'|'danger'='neutral'){
    setNotice({title,message,confirm,action,actionTone,cancelTone});
  }
  ...
  async function emailSignIn(event:FormEvent){
    ...
    if(authMode==='signup'&&!namePartsWithinLimit(f,l)){setNotice({title:'Name is too long',message:`First and last names can each contain up to ${NAME_CHARACTER_LIMIT} characters.`});return;}
  ...
  function setPermissionPlayer(player:Player|null){if(!player){setPermissionPlayerState(null);return;}if(host&&!admin){confirmHostChange(player);return;}setPermissionPlayerState(player)}
  function chooseRestriction(player:Player){setPermissionPlayerState(null);confirmRestriction(player)}
  ...
  async function adminAddPlayer(event:FormEvent){
    ...
    if(!namePartsWithinLimit(f,l)){setNotice({title:'Name is too long',message:`First and last names can each contain up to ${NAME_CHARACTER_LIMIT} characters.`});return;}
  ...
  function previewPlayerSubstitute(){
    ...
    ask(`Swap with ${target.display_name}?`,`This will send ${target.display_name} a request to permanently swap positions with you.`,'Continue',async()=>{if(await rpc('request_player_substitute',{p_target_id:target.id},false)){cancelSubstitute();setNotice({title:'Swap request sent',message:`Your request was sent to ${target.display_name}.`});}},'success','danger');
  }
  ...
  function startTeamSubInvite(team:KingTeam){
    setNotice({title:'Invite a Sub',message:'Select a player to invite as a sub. They must accept before becoming a substitute.',confirm:'Continue',actionTone:'success',cancelLabel:'Cancel',cancelTone:'danger',showBack:true,action:async()=>{setInviteSubTargetId(null);setInviteSubTeamId(team.id)}});
  }
  function selectTeamSubTarget(player:Player){
    const team=kingTeams.find(item=>item.id===inviteSubTeamId);if(!team)return;
    if(player.team_id===team.id){setInviteSubTargetId(null);setNotice({title:'Cannot select this player',message:'You cannot select someone who is on your team.'});return;}
    setInviteSubTargetId(current=>current===player.id?null:player.id);
  }
  function confirmTeamSubTarget(){
    ...
    const teamLabel=displayedTeamLabel(team,kingTeams,config.court_count);
  ...
  async function answerTeamSubstitute(id:string,accept:boolean,teamLabel:string){const succeeded=await rpc('answer_team_substitute',{p_request_id:id,p_accept:accept},!accept);if(succeeded&&accept)setNotice({title:'Done',message:`You are now a substitute for ${teamLabel}.`});}
  ...
  useEffect(()=>{
    const incoming=substituteRequests.find(request=>!handledSwapRequestIds.current.has(request.id)&&players.find(player=>player.id===request.target_id)?.user_id===user?.id);if(!incoming)return;
    handledSwapRequestIds.current.add(incoming.id);
    setNotice(existing=>{if(existing?.requestId===`substitute:${incoming.id}`)return existing;const requester=incoming.requester?.display_name??'A player';return{requestId:`substitute:${incoming.id}`,blocking:true,title:'Permanent swap request',message:`${requester} wants to swap positions with you. Accepting permanently swaps your positions.`,confirm:'Accept',actionTone:'success',action:async()=>{await answerSubstitute(incoming.id,true)},cancelLabel:'Decline',cancelTone:'danger',cancelAction:async()=>{await answerSubstitute(incoming.id,false)}};});
  ...
  useEffect(()=>{
    ...
    const team=kingTeams.find(item=>item.id===incoming.team_id);if(!team)return;
    handledTeamSubRequestIds.current.add(incoming.id);
    const teamLabel=displayedTeamLabel(team,kingTeams,config.court_count);
    setNotice(existing=>{if(existing?.requestId===`team-sub:${incoming.id}`)return existing;return{requestId:`team-sub:${incoming.id}`,blocking:true,title:'Substitute invitation',message:`${teamLabel} has invited you to play as their substitute. If you accept, you will leave your current team and become a substitute for ${teamLabel}.`,confirm:'Accept',actionTone:'success',action:async()=>{await answerTeamSubstitute(incoming.id,true,teamLabel)},cancelLabel:'Decline',cancelTone:'danger',cancelAction:async()=>{await answerTeamSubstitute(incoming.id,false,teamLabel)}};});
  },[teamSubstituteRequests,players,kingTeams,user?.id,config.court_count]);
  useEffect(()=>{
  ...
  async function advanceGame(courtNumber=me?.court_number??courts[0]?.court_number??1,confirmedGame?:number){
  ...
  function confirmJoinKingTeam(team:KingTeam,side:number){
    if(!me||(meHasActiveStatus&&team.id===me.team_id))return;
  ...
  function askKingWinner(courtNumber:number,confirmedGame=courts.find(item=>item.court_number===courtNumber)?.game_number){
    ...
    if(operator){
      setNotice({title:`Which team won on Court ${courtNumber}?`,message:'Select the team that won this game.',confirm:active[0].name,actionTone:'success',action:()=>recordKingWinner(courtNumber,active[0].id,confirmedGame),cancelLabel:active[1].name,cancelTone:'success',cancelAction:()=>recordKingWinner(courtNumber,active[1].id,confirmedGame),blocking:true,showBack:true});
      return;
    }
    const own=active.find(team=>team.id===me?.team_id);const opponent=active.find(team=>team.id!==me?.team_id);
    if(!own||!opponent)return;
    setNotice({title:'King of the Court Winner',message:`Did your team win? (You are ${own.name}).\n\nThis will record the results for ${own.name} vs ${opponent.name} on Court ${courtNumber}.`,confirm:'Yes',actionTone:'success',action:()=>recordKingWinner(courtNumber,own.id,confirmedGame),cancelLabel:'No',cancelTone:'danger',cancelAction:()=>recordKingWinner(courtNumber,opponent.id,confirmedGame),blocking:true});
  }
  function confirmKingNext(courtNumber:number){
    const court=courts.find(item=>item.court_number===courtNumber);
    const confirmedGame=court?.game_number;
    if((court?.team_mode??'rotation')==='rotation'){
      setNotice(operator?{title:`End the game on Court ${courtNumber}?`,message:'Both teams will rotate out and the next two waiting teams will enter.',confirm:'Continue',actionTone:'success',action:()=>rotateTeamCourt(courtNumber,confirmedGame),cancelLabel:'Cancel',cancelTone:'danger'}:{title:'Next Game',message:'Make sure the game is finished before continuing — this will notify everyone that you advanced the next game.',confirm:'Continue',actionTone:'success',action:()=>rotateTeamCourt(courtNumber,confirmedGame),cancelLabel:'Cancel',cancelTone:'danger'});
      return;
    }
    if(operator){askKingWinner(courtNumber,confirmedGame);return;}
    setNotice({title:'Next Game',message:'Make sure the game is finished before continuing — this will notify everyone that you advanced the next game.',confirm:'Continue',actionTone:'success',action:async()=>askKingWinner(courtNumber,confirmedGame),cancelLabel:'Cancel',cancelTone:'danger'});
  }
  ...
  if(screen==='name')return <Shell><section className="auth-card"><button className="back" onClick={()=>setScreen('welcome')}>← Go back</button><h1>What should we call you?</h1><p>Your name is added to the waitlist as soon as you press “Join waitlist”</p><form onSubmit={join}><label><span className="field-label">First name <span className="required-marker">*</span></span><input required maxLength={NAME_CHARACTER_LIMIT} value={first} onChange={e=>setFirst(e.target.value)} placeholder="First name"/><NameLimitCounter value={first}/></label><label><span className="field-label">Last initial or name <span className="optional-marker">(Optional)</span></span><input maxLength={NAME_CHARACTER_LIMIT} value={last} onChange={e=>setLast(e.target.value)} placeholder="Last initial or name"/><NameLimitCounter value={last}/></label><button className="hero-button" disabled={busy}>Join waitlist</button></form></section>{notice&&<Modal notice={notice} close={()=>setNotice(null)} busy={busy}/>}</Shell>;

  const ownKingTeam=isTeamsMode(config.mode)&&meIsVisibleInQueue?(ownTeamSubstitute?kingTeams.find(team=>team.id===ownTeamSubstitute.team_id):me?.team_id?kingTeams.find(team=>team.id===me.team_id):null):null;
  const ownTeamFillIn=me?teamFillIns.find(fill=>fill.filler_id===me.id):undefined;
  const fillInDestination=ownTeamFillIn?kingTeams.find(team=>team.id===ownTeamFillIn.destination_team_id):undefined;
  const ownKingCourt=fillInDestination?.status==='current'?fillInDestination.court_number:ownKingTeam?.status==='current'&&!ownTeamFillIn?ownKingTeam.court_number:null;
  const teamsPlayerSittingOut=me?.status==='sitout';
  const teamsPlayerFillingIn=Boolean(ownTeamFillIn);
  ...
  return <Shell>
    ...
    <main className={`queue-page ${!admin&&me&&!meIsVisibleInQueue?'rejoin-only-queue':''}`}>
      ...
      {isTeamsMode(config.mode)&&<KingBoard teams={kingTeams} courts={courts} me={me} admin={admin} host={host} busy={busy} fillIns={teamFillIns} teamSubstitutes={teamSubstitutes} inviteSubTeamId={inviteSubTeamId} players={players} fillIn={confirmTeamFillIn} unfill={cancelTeamFillIn} inviteSub={startTeamSubInvite} selectSubTarget={selectTeamSubTarget} cancelSubInvite={()=>setInviteSubTeamId(null)} removeSub={removeTeamSubstitute} nextGame={confirmTeamNext} joinTeam={confirmJoinKingTeam} joinEmptyTeam={confirmCreateKingTeam} setCourtRules={setTeamCourtRules} movePlayer={moveKingPlayer} adminSitOut={confirmAdminSitOut} adminLeave={confirmAdminLeave} permissions={setPermissionPlayer}/>} 
      {isTeamsMode(config.mode)&&!admin&&me&&meIsVisibleInQueue&&['current','waiting','sitout'].includes(me.status)&&<section className="my-actions"><span>Your actions</span><div><div className={`next-game-control ${!canPlayerAdvanceKing&&(!(onboarding==='tutorial'&&tutorialStep===0)||teamsPlayerSittingOut)?'unavailable':''}`}><button className="next next-with-substitute" disabled={busy||(!canPlayerAdvanceKing&&(!(onboarding==='tutorial'&&tutorialStep===0)||teamsPlayerSittingOut))} onClick={()=>{if(ownKingCourt)confirmKingNext(ownKingCourt)}}>Next game{courts.length>1&&ownKingCourt?` (Court ${ownKingCourt})`:''}</button>{!canPlayerAdvanceKing&&!teamsPlayerSittingOut&&!(onboarding==='tutorial'&&tutorialStep===0)&&<small>Only available when your team is in a current game</small>}</div><button className={`substitute-action ${teamsPlayerSittingOut?'teams-action-locked':''}`} disabled={busy||teamsPlayerSittingOut} onClick={playerSubstituting?cancelSubstitute:startPlayerSubstitute}>{playerSubstituting?'Cancel swap':'Swap'}</button><button className={`neutral ${teamsPlayerFillingIn?'teams-action-locked':''}`} disabled={busy||teamsPlayerFillingIn} onClick={confirmMySitOut}>{teamsPlayerSittingOut?'Unsit':'Sit out'}</button><button className={`danger ${teamsPlayerSittingOut?'teams-action-locked':''}`} disabled={busy||teamsPlayerSittingOut} onClick={()=>ask('Leave the waitlist?','This removes you from the current game or queue.','Leave',leaveOwnWaitlist)}>Leave</button></div></section>}
      {isTeamsMode(config.mode)&&!admin&&me&&(!meIsVisibleInQueue||!['current','waiting','sitout'].includes(me.status))&&<section className="my-actions"><span>Your actions</span><div><button className="rejoin-at-back top-rejoin-action" disabled={busy} onClick={()=>void rejoinAtBack()}>Rejoin</button></div></section>}
      {admin&&<div className="facility-admin-context"><span>Managing</span><strong>{facility?.name}</strong></div>}
      {isTeamsMode(config.mode)&&operator&&<section className={`admin-tools teams-tools ${courts.length===1?'single-court-tools':'multi-court-tools'} ${host?'host-tools':''} ${host&&teamsPlayerSittingOut?'host-sitting-out':''}`} inert={host&&teamsPlayerSittingOut||undefined}>{host&&<span className="host-actions-title">Host Actions</span>}{admin&&<select value={config.mode} onChange={e=>void changeWaitlistMode(e.target.value as Config['mode'])}><option value="regular">Regular waitlist</option><option value="rejoin">Rejoin waitlist</option><option value="teams">Teams mode</option><option value="teams_rejoin">Teams mode (Rejoin)</option></select>}{courts.length===1&&<button className="next-game-tool" disabled={busy||kingTeams.filter(team=>team.status==='current'&&team.court_number===(courts[0]?.court_number??1)).length!==2} onClick={()=>confirmKingNext(courts[0]?.court_number??1)}>Next game</button>}<button className="add-player-tool" onClick={()=>setScreen('add-player')}>Add player</button>{admin&&<button className="facility-tool" title={`Change the required player location for ${facility?.name??'this facility'}`} onClick={()=>void setFacilityLocation()}>{config.geofence_enabled?'Update facility location':'Set facility location'}{facility?.code?` (${facility.code})`:''}</button>}<button className={adminRejoins.length?'rejoin-tool attention':'rejoin-tool'} aria-label={adminRejoins.length?`Rejoin requests, ${adminRejoins.length} pending`:'Rejoin requests'} onClick={()=>setScreen('offline-rejoin')}>Rejoin requests{adminRejoins.length>0&&<span className="notification-badge" aria-hidden="true">{adminRejoins.length>99?'99+':adminRejoins.length}</span>}</button><button className={`group-create-tool ${adminGrouping?'active':''}`} onClick={adminGrouping?cancelAdminGrouping:startAdminGrouping}>{adminGrouping?'Cancel Grouping':'Create Group'}</button><div className="undo-redo-controls" aria-label="Undo and redo"><button aria-label="Undo last session action" onClick={()=>void rpc('admin_undo_last')}>Undo</button><button aria-label="Redo last undone session action" onClick={()=>void rpc('admin_redo_last')}>Redo</button></div><button className={`substitute-tool ${adminSubstituting?'active':''}`} onClick={adminSubstituting?cancelSubstitute:startAdminSubstitute}>{adminSubstituting?'Cancel swap':'Swap'}</button>{admin&&<button className="history-tool" onClick={()=>void openAdminHistory()}>History</button>}{admin&&<button className="members-tool" onClick={()=>void openMembers()}>Members</button>}{admin&&<button className="reset-tool" onClick={()=>ask('Reset the entire waitlist?','This removes every player and clears past games.','Reset waitlist',async()=>{await rpc('admin_reset_waitlist')})}>Reset waitlist</button>}</section>}
      {!isTeamsMode(config.mode)&&!admin&&me&&(!meIsVisibleInQueue||!['current','waiting','sitout'].includes(me.status))&&<section className="my-actions"><span>Your actions</span><div><button className="rejoin-at-back top-rejoin-action" disabled={busy} onClick={()=>void rejoinAtBack()}>Rejoin</button></div></section>}
      {!isTeamsMode(config.mode)&&!admin&&me&&meIsVisibleInQueue&&['current','waiting','sitout'].includes(me.status)&&<section className={`my-actions ${onboarding==='tutorial'&&tutorialStep===0&&me.status!=='current'?'tutorial-focus':''}`}><span>Your actions</span><div><div className={`next-game-control ${me.status!=='current'&&!(onboarding==='tutorial'&&tutorialStep===0)?'unavailable':''}`}><button className={`next ${me.status==='current'?'next-with-substitute':''} ${onboarding==='tutorial'&&tutorialStep===0?'tutorial-focus':''}`} disabled={busy||me.restricted||(me.status!=='current'&&!(onboarding==='tutorial'&&tutorialStep===0))} aria-disabled={me.status!=='current'&&!(onboarding==='tutorial'&&tutorialStep===0)} onClick={()=>{if(me.status!=='current')return;const courtNumber=me.court_number??courts[0]?.court_number??1;const gameNumber=courts.find(court=>court.court_number===courtNumber)?.game_number;ask('Start the next game?',`This will notify all players that ${me.display_name} advanced the queue. This cannot be quietly undone.`,'Next game',()=>advanceGame(courtNumber,gameNumber),'success')}}>Next game</button>{me.status!=='current'&&!(onboarding==='tutorial'&&tutorialStep===0)&&<small>Only available when in a current game</small>}</div><button className="substitute-action" disabled={busy} onClick={playerSubstituting?cancelSubstitute:startPlayerSubstitute}>{playerSubstituting?'Cancel swap':'Swap'}</button><button className={`neutral ${onboarding==='tutorial'&&tutorialStep===1+(config.mode==='rejoin'?1:0)?'tutorial-focus':''}`} disabled={busy} onClick={confirmMySitOut}>Sit out</button><button className={`danger ${onboarding==='tutorial'&&tutorialStep===2+(config.mode==='rejoin'?1:0)?'tutorial-focus':''}`} disabled={busy} onClick={()=>ask('Leave the waitlist?','This removes you from the current game or queue.','Leave',leaveOwnWaitlist)}>Leave</button></div></section>}
      {!isTeamsMode(config.mode)&&operator&&<section className={`admin-tools standard-tools ${courts.length===1?'single-court-tools':'multi-court-tools'} ${host?'host-tools':''}`}>{host&&<span className="host-actions-title">Host Actions</span>}{admin&&<select value={config.mode} onChange={e=>void changeWaitlistMode(e.target.value as Config['mode'])}><option value="regular">Regular waitlist</option><option value="rejoin">Rejoin waitlist</option><option value="teams">Teams mode</option><option value="teams_rejoin">Teams mode (Rejoin)</option></select>}<button className="next-game-tool" disabled={busy||current.length===0} onClick={()=>{const courtNumber=courts[0]?.court_number??1;const gameNumber=courts.find(court=>court.court_number===courtNumber)?.game_number;ask('Start the next game?','This will notify all players and advance the entire queue to the next game.','Next game',()=>advanceGame(courtNumber,gameNumber))}}>Next game</button><button className="add-player-tool" onClick={()=>setScreen('add-player')}>Add player</button>{admin&&<button className="facility-tool" title={`Change the required player location for ${facility?.name??'this facility'}`} onClick={()=>void setFacilityLocation()}>{config.geofence_enabled?'Update facility location':'Set facility location'}{facility?.code?` (${facility.code})`:''}</button>}<button className={adminRejoins.length?'rejoin-tool attention':'rejoin-tool'} aria-label={adminRejoins.length?`Rejoin requests, ${adminRejoins.length} pending`:'Rejoin requests'} onClick={()=>setScreen('offline-rejoin')}>Rejoin requests{adminRejoins.length>0&&<span className="notification-badge" aria-hidden="true">{adminRejoins.length>99?'99+':adminRejoins.length}</span>}</button><button className={`group-create-tool ${adminGrouping?'active':''}`} onClick={adminGrouping?cancelAdminGrouping:startAdminGrouping}>{adminGrouping?'Cancel Grouping':'Create Group'}</button><div className="undo-redo-controls" aria-label="Undo and redo"><button aria-label="Undo last session action" onClick={()=>void rpc('admin_undo_last')}>Undo</button><button aria-label="Redo last undone session action" onClick={()=>void rpc('admin_redo_last')}>Redo</button></div><button className={`substitute-tool ${adminSubstituting?'active':''}`} onClick={adminSubstituting?cancelSubstitute:startAdminSubstitute}>{adminSubstituting?'Cancel swap':'Swap'}</button>{admin&&<button className="history-tool" onClick={()=>void openAdminHistory()}>History</button>}{admin&&<button className="members-tool" onClick={()=>void openMembers()}>Members</button>}{admin&&<button className="reset-tool" onClick={()=>ask('Reset the entire waitlist?','This removes every player and clears past games. The admin can undo this action.','Reset waitlist',async()=>{await rpc('admin_reset_waitlist')})}>Reset waitlist</button>}</section>}
      {!isTeamsMode(config.mode)&&courts.map((court,courtIndex)=>{const courtPlayers=current.filter(player=>player.court_number===court.court_number);const hiddenDuringDrag=draggedCourtNumber!==null&&court.court_number!==draggedCourtNumber;return <section className={`court-section ${hiddenDuringDrag?'drag-hidden-court':''}`} aria-hidden={hiddenDuringDrag||undefined} key={court.court_number}>{operator&&<button className="court-next-button" disabled={busy||courtPlayers.length===0} onClick={()=>{const courtNumber=court.court_number;const gameNumber=court.game_number;ask(`Start next game on Court ${courtNumber}?`,`Only Court ${courtNumber} will advance and refill from the shared waitlist.`,'Next game',()=>advanceGame(courtNumber,gameNumber))}}>Next game (Court {court.court_number})</button>}<QueueCard title={courts.length>1?`COURT ${court.court_number} - Game ${court.game_number}`:`Game ${court.game_number}`} subtitle={`${courtPlayers.length} playing`} status="current" players={courtPlayers} start={1} savedQueuePositions={savedQueuePositions} me={me} admin={admin} operator={operator} spotlight={onboarding==='tutorial'&&tutorialStep===3+(config.mode==='rejoin'?1:0)&&courtIndex===0} editing={editing} editName={editName} setEditing={setEditing} setEditName={setEditName} saveName={saveName} requestGroup={requestGroup} leaveGroup={removeGroupMember} leaveOwnGroup={confirmLeaveOwnGroup} adminLeaveGroup={adminRemoveGroupMember} permissions={setPermissionPlayer} adminSitOut={confirmAdminSitOut} adminLeave={confirmAdminLeave} dragging={dragging} dragOver={dragOver} setDragging={setDragging} setDragOver={setDragOver} movePlayer={movePlayer}/></section>})}
      {!isTeamsMode(config.mode)&&<QueueCard title="Waitlist" subtitle={tutorialWaiting.length?`${tutorialWaiting.length} waiting`:'No one waiting'} status="waiting" players={tutorialWaiting} start={13} savedQueuePositions={savedQueuePositions} me={me} admin={admin} operator={operator} spotlight={onboarding==='tutorial'&&tutorialStep===4+(config.mode==='rejoin'?1:0)} groupSpotlight={onboarding==='tutorial'&&tutorialStep===5+(config.mode==='rejoin'?1:0)} editing={editing} editName={editName} setEditing={setEditing} setEditName={setEditName} saveName={saveName} projections={projectedGames} projectedCourts={courts.length>1?new Map([...projections].map(([id,value])=>[id,value.court])):undefined} requestGroup={requestGroup} leaveGroup={removeGroupMember} leaveOwnGroup={confirmLeaveOwnGroup} adminLeaveGroup={adminRemoveGroupMember} permissions={setPermissionPlayer} adminSitOut={confirmAdminSitOut} adminLeave={confirmAdminLeave} dragging={dragging} dragOver={dragOver} setDragging={setDragging} setDragOver={setDragOver} movePlayer={movePlayer}/>}
    ...
    {permissionPlayer&&<PermissionsModal player={permissionPlayer} admin={admin} close={()=>setPermissionPlayer(null)} hostAction={()=>confirmHostChange(permissionPlayer)} restrictAction={()=>chooseRestriction(permissionPlayer)}/>}
    {hostTutorial&&<><TutorialDemo mode={config.mode} host/><HostTutorial mode={config.mode} step={hostTutorialStep} next={()=>hostTutorialStep<getHostTutorialSteps(config.mode).length-1?setHostTutorialStep(value=>value+1):setHostTutorial(false)} back={()=>setHostTutorialStep(value=>Math.max(0,value-1))} skip={()=>setHostTutorial(false)}/></>}
...
function WaitlistDisclaimer({mode,language,acknowledge}:{mode:Config['mode'];language:AppLanguage;acknowledge:()=>void}){
 const rejoin=mode==='rejoin'||mode==='teams_rejoin';const teams=isTeamsMode(mode);
 const title=language==='es'?`Esta es una lista ${teams?'POR EQUIPOS':''}${rejoin?' DE REINGRESO':teams?'':'REGULAR'}`:language==='zh-CN'?`这是${teams?'团队':''}${rejoin?'重新加入':'普通'}等候名单`:`This is a ${teams?'TEAMS MODE':rejoin?'REJOIN':'REGULAR'}${mode==='teams_rejoin'?' (REJOIN)':''} waitlist`;
 const details=language==='es'?(rejoin?(teams?'Después de que tu equipo salga de la cancha, DEBES volver a ingresar dentro de cinco minutos para conservar tu lugar.':'Después de cada juego, DEBES volver a ingresar dentro de cinco minutos para conservar tu lugar.'):'No necesitas volver a registrarte. Permanecerás en la fila hasta que salgas o estés fuera del rango permitido del centro.'):language==='zh-CN'?(rejoin?(teams?'球队离开球场后，你必须在五分钟内重新加入，才能保留你的位置。':'每次比赛结束后，你必须在五分钟内重新加入，才能保留你的位置。'):'你无需重新报名。除非你主动离开或超出场馆允许的范围，否则你会一直保留在队列中。'):(rejoin?(teams?'After your team rotates off the court, you MUST rejoin within five minutes to keep your place.':'Every time after you play, you MUST rejoin within five minutes to keep your place.'):'You do not need to sign up again. You stay in line until you leave or move out of range of the facility.');
...
const tutorialSteps=[
 {title:'Next Game action button',message:'This button appears only when you are in the current game. Pressing it ends that game, advances the entire queue, and notifies every player—so use it only when the game is truly over.',target:'.my-actions .next'},
 {title:'Sit Out action button',message:'Sit Out makes you skip one game. After skipping that game, you receive priority for the following game.',target:'.my-actions .neutral'},
 {title:'Leave action button',message:'Leave removes only you from the waitlist, so use it when you do not want to play anymore.',target:'.my-actions .danger'},
 {title:'Who is playing now',message:'The players in this section are in the current game. Only they are able to start the next game.',target:'.court-section .queue-card'},
 {title:'Waitlist',message:'Your number is your queue position. When you are waiting, your projected game appears beside your name.',target:'[data-drop-status="waiting"]'},
 {title:'Group Up',message:'Use Group Up beside another player when you want to play together.',target:'.tutorial-group-target'},
];
...
const hostTutorialSteps=[
 ...
 {title:'Swap',message:'Use Swap to select two players and permanently swap their positions.',target:'.substitute-tool'},
...
const teamHostTutorialSteps=[
 {title:'Next Game',message:'Advance the correct court here. For King of the Court, select the winning team; for 2 on, 2 off, both teams rotate while remaining intact.',target:'.next-game-tool'},
 {title:'Court Format',message:'Choose 2 on, 2 off or King of the Court separately for each court, including the maximum game streak for King of the Court.',target:'.court-team-rule'},
 {title:'Add Player',message:'Use Add Player to check in someone who does not have their phone. They enter the first available team spot.',target:'.add-player-tool'},
 {title:'Rejoin Requests',message:'Rejoin Requests are usually for players without a phone who joined through you. After they play, they can ask you to help them rejoin.',target:'.rejoin-tool'},
 {title:'Move Players',message:'Press and hold a player on mobile, or drag them on desktop, to move them into any team with an open spot.',target:'.king-team-card .player-row'},
 {title:'Swap',message:'Use Swap to select two players and permanently swap their positions. Their teams stay unchanged unless the swap moves them to another team.',target:'.substitute-tool'},
 {title:'Sit Out and Remove',message:'Use Sit Out beside a player to skip that player for one game. Use Remove to take that player out of the waitlist.',target:'.admin-player-actions'},
 {title:'Action History',message:'Action History records the player and queue changes made during this waitlist.',target:'.your-history-button'},
];
function getHostTutorialSteps(mode:Config['mode']){return isTeamsMode(mode)?teamHostTutorialSteps:hostTutorialSteps}

function DemoPlayer({position,name,own=false}:{position:number;name:string;own?:boolean}){return <article className={`player-row ${own?'own':''}`}><span className="position">{position}</span><div className="player-name"><strong>{name}</strong></div>{own&&<span className="you">You</span>}</article>}
function DemoHostPlayer({position,name,own=false}:{position:number;name:string;own?:boolean}){return <article className={`player-row admin-row ${own?'own':''}`}><span className="position">{position}</span><div className="player-name"><strong>{name}{own?' (Host)':''}</strong></div>{own&&<span className="you">You</span>}<div className="admin-player-actions"><button className="admin-sitout-button">Sit Out</button><button className="admin-leave-button">Remove</button>{!own&&<button className="restrict-button">Permissions</button>}</div></article>}
function TutorialDemo({mode,host=false}:{mode:Config['mode'];host?:boolean}){
 const teams=isTeamsMode(mode);
 return <section className={`tutorial-mode-demo ${host?'tutorial-host-demo queue-page':''}`} aria-hidden="true"><div className="tutorial-demo-scroll"><header className={host?'topbar tutorial-demo-header':'tutorial-demo-header'}><Logo compact/>{host?<div className="top-actions"><button className="icon-button">Log out</button><span className="language-picker tutorial-language"><span className="language-symbol"><i>🌐</i><b>ENG</b></span></span></div>:<span className="live-pill"><i/>Demo</span>}</header><section className="game-heading"><div><span className="kicker">{host?'LIVE QUEUE · HOST':'TUTORIAL QUEUE'}</span><h1>Game 4</h1></div><span className="live-pill"><i/>Live</span></section>
 {host&&<section className={`admin-tools host-tools single-court-tools ${teams?'teams-tools':'standard-tools'}`}><span className="host-actions-title">Host Actions</span><button className="next-game-tool">Next game</button><button className="add-player-tool">Add player</button><button className="rejoin-tool">Rejoin requests</button>{!teams&&<button className="group-create-tool">Create Group</button>}<div className="undo-redo-controls" aria-label="Undo and redo"><button>Undo</button><button>Redo</button></div><button className="substitute-tool">Swap</button></section>}
 {!host&&<section className="my-actions"><span>Your actions</span><div><button className="next next-with-substitute">Next game</button><button className="substitute-action">Swap</button><button className="neutral">Sit Out</button><button className="danger">Leave</button></div></section>}
 {teams?<><section className="queue-card king-court court-section"><header><div className="king-court-title"><h2>CURRENT GAME - Team 1 vs Team 2</h2><div className="court-team-rule"><span className="king-rule-display"><b>KING OF THE COURT</b><small>(2 CONSECUTIVE GAMES MAX)</small></span></div></div></header><div className="king-current-teams"><section className="king-team-card own-king-team"><div className="team-block-label"><strong>Team 1</strong>{!host&&<button className="king-join-button king-own-team-label">Your team</button>}</div>{host?<><DemoHostPlayer position={1} name="You" own/><DemoHostPlayer position={2} name="Demo teammate"/></>:<><DemoPlayer position={1} name="You" own/><DemoPlayer position={2} name="Demo teammate"/></>}</section><section className="king-team-card"><div className="team-block-label"><strong>Team 2</strong>{!host&&<button className="king-join-button">Join +</button>}</div>{host?<><DemoHostPlayer position={7} name="Jordan"/><DemoHostPlayer position={8} name="Casey"/></>:<><DemoPlayer position={7} name="Jordan"/><DemoPlayer position={8} name="Casey"/></>}</section></div></section><section className="king-waitlist queue-card"><header><h2>Waitlist</h2><span>1 team waiting</span></header><section className="king-team-card"><div className="team-block-label"><strong>Team 3</strong>{!host&&<button className="king-join-button">Join +</button>}</div>{host?<><DemoHostPlayer position={13} name="Taylor"/><DemoPlayer position={14} name="Open spot"/></>:<><DemoPlayer position={13} name="Taylor"/><DemoPlayer position={14} name="Open spot"/></>}</section></section></>:<><section className="court-section"><section className="queue-card"><header><h2>Game 4</h2><span>2 playing</span></header>{host?<><DemoHostPlayer position={1} name="You" own/><DemoHostPlayer position={2} name="Jordan"/></>:<><DemoPlayer position={1} name="You" own/><DemoPlayer position={2} name="Jordan"/></>}</section></section><section className="queue-card" data-drop-status="waiting"><header><h2>Waitlist</h2><span>2 waiting</span></header>{host?<><DemoHostPlayer position={13} name="Casey"/><DemoHostPlayer position={14} name="Taylor"/></>:<><article className="player-row"><span className="position">13</span><div className="player-name"><strong>Casey</strong></div><button className="group-button tutorial-group-target">Group Up</button></article><DemoPlayer position={14} name="Taylor"/></>}</section></>}
...
function keepTutorialTargetClear(target:HTMLElement,coachSelector:string){const scroller=target.closest('.tutorial-demo-scroll') as HTMLElement|null;if(!scroller)return;const coach=document.querySelector(coachSelector) as HTMLElement|null;if(!coach)return;const targetBox=target.getBoundingClientRect(),coachBox=coach.getBoundingClientRect(),gap=22;if(coachBox.top>window.innerHeight/2&&targetBox.bottom>coachBox.top-gap)scroller.scrollBy({top:targetBox.bottom-(coachBox.top-gap),behavior:'smooth'});else if(coachBox.bottom<window.innerHeight/2&&targetBox.top<coachBox.bottom+gap)scroller.scrollBy({top:targetBox.top-(coachBox.bottom+gap),behavior:'smooth'})}
function HostTutorial({mode,step,next,back,skip}:{mode:Config['mode'];step:number;next:()=>void;back:()=>void;skip:()=>void}){useTutorialInteractionLock();const steps=getHostTutorialSteps(mode);const safeStep=Math.max(0,Math.min(step,steps.length-1));const item=steps[safeStep];const [placement,setPlacement]=useState<'top'|'bottom'>('bottom');useEffect(()=>{document.querySelectorAll('.host-tutorial-focus').forEach(node=>node.classList.remove('host-tutorial-focus'));if(!item?.target)return;const target=document.querySelector(`.tutorial-mode-demo ${item.target}`) as HTMLElement|null;if(!target)return;target.classList.add('host-tutorial-focus');target.scrollIntoView({behavior:'auto',block:'center'});setPlacement(target.getBoundingClientRect().top<window.innerHeight/2?'bottom':'top');return()=>target.classList.remove('host-tutorial-focus')},[item?.target]);useEffect(()=>{if(!item?.target)return;const target=document.querySelector(`.tutorial-mode-demo ${item.target}`) as HTMLElement|null;if(!target)return;const frame=requestAnimationFrame(()=>requestAnimationFrame(()=>keepTutorialTargetClear(target,'.host-tutorial')));return()=>cancelAnimationFrame(frame)},[item?.target,placement]);if(!item)return null;return <><div className="tutorial-scrim"/><section className={`tutorial-coach host-tutorial host-tutorial-${placement}`} role="dialog" aria-modal="true"><div className="tutorial-progress"><span>HOST TOUR · {safeStep+1} OF {steps.length}</span><button onClick={skip}>Skip tutorial</button></div><h2>{item.title}</h2><p>{item.message}</p><div className="tutorial-actions">{safeStep>0&&<button className="neutral" onClick={back}>Back</button>}<button className="next" onClick={next}>{safeStep===steps.length-1?'Finish':'Next'}</button></div></section></>}
function TutorialCoach({mode,step,next,back,skip}:{mode:Config['mode'];step:number;next:()=>void;back:()=>void;skip:()=>void}){useTutorialInteractionLock();const steps=getTutorialSteps(mode);const safeStep=Math.max(0,Math.min(step,steps.length-1));const item=steps[safeStep];const demo='demo'in item&&item.demo;const [placement,setPlacement]=useState<'top'|'bottom'>('bottom');useEffect(()=>{if(demo)return;const selector='target'in item?item.target:null;if(!selector)return;const target=document.querySelector(`.tutorial-mode-demo ${selector}`) as HTMLElement|null;if(!target)return;target.classList.add('tutorial-focus');target.scrollIntoView({behavior:'auto',block:'center'});const place=target.getBoundingClientRect().top<window.innerHeight/2?'bottom':'top';setPlacement(place);const frame=requestAnimationFrame(()=>requestAnimationFrame(()=>keepTutorialTargetClear(target,'.tutorial-coach:not(.host-tutorial)')));return()=>{cancelAnimationFrame(frame);target.classList.remove('tutorial-focus')}},[safeStep,demo,item]);return <><div className="tutorial-scrim"/><div className={demo?'tutorial-rejoin-tour':'tutorial-standard-tour'}>{demo&&<section className="tutorial-rejoin-demo" aria-hidden="true"><Logo compact/><span className="kicker">REJOIN WAITLIST</span><h2>Do you want to rejoin?</h2><p>Your position is saved.</p><div className="return-countdown rejoin-countdown"><strong>4:59</strong><span>left to rejoin</span></div><div className="rejoin-actions"><button className="next tutorial-rejoin-highlight" disabled>Rejoin</button><button className="danger" disabled>Leave</button></div></section>}{demo&&<span className="tutorial-rejoin-arrow" aria-hidden="true">↑</span>}<section className={`tutorial-coach tutorial-coach-${placement} ${demo?'tutorial-coach-rejoin-demo':''}`} role="dialog" aria-modal="true" aria-live="polite"><div className="tutorial-progress"><span>QUICK TOUR · {safeStep+1} OF {steps.length}</span><button onClick={skip}>Skip tutorial</button></div><h2>{item.title}</h2><p>{item.message}</p><div className="tutorial-actions">{safeStep>0&&<button className="neutral" onClick={back}>Back</button>}<button className="next" onClick={next}>{safeStep===steps.length-1?'Finish':'Next'}</button></div></section></div></>}
function createPlayerDragPreview(player:Player,players:Player[],start:number){
 ...
 const preview=document.createElement('div');preview.className='admin-drag-preview';preview.setAttribute('aria-hidden','true');preview.style.cssText='position:fixed;display:block;visibility:visible;opacity:.98;z-index:2147483647;pointer-events:none;';
 for(const movingPlayer of movingPlayers){
  const source=document.querySelector<HTMLElement>(`[data-player-id="${movingPlayer.id}"]`);
  if(source){const row=source.cloneNode(true) as HTMLElement;row.removeAttribute('data-player-id');row.removeAttribute('draggable');row.classList.remove('dragging');row.classList.add('admin-drag-preview-row');row.style.visibility='visible';row.querySelectorAll<HTMLElement>('*').forEach(child=>{child.style.visibility='visible'});row.querySelectorAll<HTMLElement>('button,input').forEach(control=>control.setAttribute('tabindex','-1'));preview.appendChild(row);}
...
function clearDragArtifacts(){document.querySelectorAll('.mobile-admin-drag-preview,.admin-drag-preview').forEach(preview=>preview.remove());document.querySelectorAll('.court-section.drag-hidden-court').forEach(section=>section.classList.remove('drag-hidden-court'));setMobileAdminDragging(false);}
...
function positionPlayerDragPreview(preview:HTMLElement,x:number,y:number){
 ...
 // appear stationary while the user's finger and the queue kept moving.
 const left=Math.min(window.innerWidth-width-edgePadding,Math.max(edgePadding,x-width/2));
...
function preserveDragScrollAwayFromEdges(y:number,scrollY:number){const edge=Math.min(88,Math.max(64,window.innerHeight*.1));if(y<=edge||y>=window.innerHeight-edge)return;scrollActiveMobileDrag(scrollY);}
...
function QueueCard({title,subtitle,status,players,start,savedQueuePositions,me,admin=false,operator=false,spotlight=false,groupSpotlight=false,editing,editName,setEditing,setEditName,saveName,requestGroup,leaveGroup,leaveOwnGroup,adminLeaveGroup,permissions,adminSitOut,adminLeave,dragging,dragOver,setDragging,setDragOver,movePlayer,projections,projectedCourts}:{title:string;subtitle:string;status:'current'|'waiting';players:Player[];start:number;savedQueuePositions:Map<string,number|null>;me?:Player|null;admin?:boolean;operator?:boolean;spotlight?:boolean;groupSpotlight?:boolean;editing:string|null;editName:string;setEditing:(v:string|null)=>void;setEditName:(v:string)=>void;saveName:(p:Player)=>void;requestGroup:(p:Player)=>Promise<void>;leaveGroup:(p:Player)=>Promise<void>;leaveOwnGroup:()=>void;adminLeaveGroup:(p:Player)=>Promise<void>;permissions:(p:Player)=>void;adminSitOut:(p:Player)=>void;adminLeave:(p:Player)=>void;dragging:string|null;dragOver:DropPlacement|null;setDragging:(v:string|null)=>void;setDragOver:(v:DropPlacement|null)=>void;movePlayer:(id:string,status:'current'|'waiting',index:number,courtNumber?:number|null)=>Promise<void>;projections?:Map<string,number>;projectedCourts?:Map<string,number>}){
 ...
 const mobileDrag=useRef<{timer:number|null;active:boolean;preview:HTMLElement|null;player:Player|null;startX:number;startY:number;lastX:number;lastY:number;scrollY:number;frame:number|null}>({timer:null,active:false,preview:null,player:null,startX:0,startY:0,lastX:0,lastY:0,scrollY:0,frame:null});
 ...
 const updateMobileTarget=(x:number,y:number)=>{const point=constrainQueueDragPoint(x,y);commitDragTarget(resolveDropPlacement(point.x,point.y,dragOverRef.current),point.y);preserveDragScrollAwayFromEdges(y,mobileDrag.current.scrollY);};
...
function KingTeamCard({team,me,canJoin,busy,joinTeam,start,side,showStreak,operator,admin,host,adminSitOut,adminLeave,permissions,dragging,drop,beginDrag,now,fillIns,players,allTeams,fillIn,unfill}:{team:KingTeam;me:Player|null;canJoin:boolean;busy:boolean;joinTeam:(team:KingTeam,side:number)=>void;start:number;side:number;showStreak:boolean;operator:boolean;admin:boolean;host:boolean;adminSitOut:(player:Player)=>void;adminLeave:(player:Player)=>void;permissions:(player:Player)=>void;dragging:string|null;drop:KingDropPlacement|null;beginDrag:(player:Player,event:ReactPointerEvent<HTMLElement>,start:number)=>void;now:number;fillIns:TeamFillIn[];players:Player[];allTeams:KingTeam[];fillIn:(sitter:Player,team:KingTeam)=>void;unfill:()=>Promise<void>}){
 ...
 const slots=[...items,...Array.from({length:Math.max(0,6-items.length)},()=>null)];
 const stableSide=Number(team.name.match(/\d+/)?.[0]??side);
 const teamRemaining=team.rejoin_expires_at?Math.max(0,Math.ceil((new Date(team.rejoin_expires_at).getTime()-now)/1000)):null;
 const joinLocked=Boolean(me&&(me.status==='sitout'||fillIns.some(fill=>fill.filler_id===me.id||fill.sitter_id===me.id)));
 const memberLabel=(item:Player)=>{const sitting=fillIns.find(fill=>fill.sitter_id===item.id);const filling=fillIns.find(fill=>fill.filler_id===item.id);const filler=sitting?players.find(player=>player.id===sitting.filler_id):null;const destination=filling?allTeams.find(candidate=>candidate.id===filling.destination_team_id):null;const badge=item.is_host?' (Host)':'';if(item.status==='sitout')return filler?<>{filler.display_name}{filler.is_host?' (Host)':''} <small className="team-player-status">(1 game replacement)</small></>:<span className="team-sitting-label"><span className="team-sitting-name">{item.display_name}{badge}</span><small>Sitting 1 game</small></span>;return <>{item.display_name}{badge}{destination&&<> <small className="team-player-status">(filling in {destination.name})</small></>}</>};
 return <section data-king-team-id={team.id} data-team-full={team.members.length>=6||undefined} className={`king-team-card queue-team-block ${own?'own-king-team':''} ${team.members.length>=6?'king-full-team':''}`}><div className="team-block-label"><strong>{team.name}{teamRemaining!==null&&` (${formatCountdown(teamRemaining)} until removed)`}</strong>{!operator&&own&&<button className="king-join-button king-own-team-label" disabled>Your team</button>}{!operator&&!own&&<button className={`king-join-button ${joinLocked&&team.members.length<6?'join-action-locked':''}`} disabled={!canJoin||busy} onClick={()=>joinTeam(team,stableSide)}>{team.members.length>=6?'Team full':'Join +'}</button>}{team.status==='current'&&showStreak&&<span>Game streak: {team.consecutive_wins}</span>}</div>{slots.map((item,index)=>item==='drop'?<article className="player-row king-player-row king-drop-placeholder" aria-label="Player drop position" key="drop-placeholder"><span className="position">{start+index}</span></article>:<article data-player-id={item?.status==='rejoin'?undefined:item?.id} className={`player-row king-player-row ${item?'':'king-empty-row'} ${item?.user_id===me?.user_id?'own':''}`} key={item?.id??`empty-${index}`} onContextMenu={event=>{if(item&&item.status!=='rejoin'&&operator)event.preventDefault()}} onPointerDown={event=>{if(item&&item.status!=='rejoin'&&operator)beginDrag(item,event,start)}}><span className="position">{start+index}</span><div className={item?.status==='rejoin'?'player-name king-player-rejoin-reservation':'player-name'}><strong>{item?.status==='rejoin'&&item.rejoin_expires_at?`${formatCountdown(Math.max(0,Math.ceil((new Date(item.rejoin_expires_at).getTime()-now)/1000)))} to rejoin`:item?memberLabel(item):'Open spot'}</strong></div>{item?.status==='sitout'&&!fillIns.some(fill=>fill.sitter_id===item.id)&&!operator&&me&&me.status!=='sitout'&&me.id!==item.id&&me.team_id!==team.id&&['current','waiting'].includes(me.status)&&!fillIns.some(fill=>fill.filler_id===me.id||fill.sitter_id===me.id)&&<button className="team-fill-in-button" disabled={busy} onClick={()=>fillIn(item,team)}>Fill in</button>}{item?.status==='sitout'&&!operator&&me&&fillIns.some(fill=>fill.sitter_id===item.id&&fill.filler_id===me.id)&&<button className="team-fill-in-button" disabled={busy} onClick={()=>void unfill()}>Unfill</button>}{item&&item.status!=='rejoin'&&item.user_id===me?.user_id&&<span className="you">You</span>}{item&&item.status!=='rejoin'&&operator&&<div className="admin-player-actions" onPointerDown={event=>event.stopPropagation()}><button className="admin-sitout-button" onClick={()=>adminSitOut(item)}>{item.status==='sitout'?'Unsit':'Sit out'}</button><button className="admin-leave-button" onClick={()=>adminLeave(item)}>Remove</button>{(admin||(host&&item.user_id!==me?.user_id&&!item.is_host&&Boolean(item.user_id)))&&<button className="restrict-button" onClick={()=>permissions(item)}>Permissions</button>}</div>}</article>)}</section>
}
function TeamSubstituteRoster({team,substitutes,expanded,canManage,operator,busy,toggle,invite,removeSub}:{team:KingTeam;substitutes:TeamSubstitute[];expanded:boolean;canManage:boolean;operator:boolean;busy:boolean;toggle:()=>void;invite:()=>void;removeSub:(substitute:TeamSubstitute)=>void}){
 const canInvite=canManage&&team.members.length>=6&&substitutes.length<6;
 if(!substitutes.length&&!canInvite)return null;
 return <section className="team-substitute-area">
  {substitutes.length>0&&<button className="team-substitute-toggle" onClick={toggle} aria-expanded={expanded}><span>{expanded?'▾':'▸'}</span> Substitutes ({substitutes.length})</button>}
  {expanded&&substitutes.length>0&&<div className="team-substitute-list">{substitutes.map((sub,index)=><article className="player-row team-substitute-player" key={sub.id}><span className="position">S{index+1}</span><div className="player-name"><strong>{sub.player?.display_name??'Substitute'}</strong></div>{operator&&<button className="admin-leave-button" onClick={()=>removeSub(sub)}>Remove</button>}</article>)}</div>}
  {canInvite&&<button className="team-add-substitute" disabled={busy} onClick={invite}>+ Add Substitutes</button>}
 </section>;
}

function KingBoard({teams,courts,me,admin,host,busy,fillIns,teamSubstitutes,inviteSubTeamId,players,fillIn,unfill,inviteSub,selectSubTarget,cancelSubInvite,removeSub,nextGame,joinTeam,joinEmptyTeam,setCourtRules,movePlayer,adminSitOut,adminLeave,permissions}:{teams:KingTeam[];courts:Court[];me:Player|null;admin:boolean;host:boolean;busy:boolean;fillIns:TeamFillIn[];teamSubstitutes:TeamSubstitute[];inviteSubTeamId:string|null;players:Player[];fillIn:(sitter:Player,team:KingTeam)=>void;unfill:()=>Promise<void>;inviteSub:(team:KingTeam)=>void;selectSubTarget:(player:Player)=>void;cancelSubInvite:()=>void;removeSub:(substitute:TeamSubstitute)=>void;nextGame:(court:number)=>void;joinTeam:(team:KingTeam,side:number)=>void;joinEmptyTeam:(side:number)=>void;setCourtRules:(court:number,mode:TeamCourtMode,maxWins:number|null)=>Promise<void>;movePlayer:(playerId:string,targetTeamId:string,index:number)=>Promise<void>;adminSitOut:(player:Player)=>void;adminLeave:(player:Player)=>void;permissions:(player:Player)=>void}){
 const operator=admin||host;
 const playerJoinLocked=Boolean(me&&(me.status==='sitout'||fillIns.some(fill=>fill.filler_id===me.id||fill.sitter_id===me.id)));
 const [dragging,setDragging]=useState<string|null>(null);
 const [drop,setDrop]=useState<KingDropPlacement|null>(null);
 const [expandedSubs,setExpandedSubs]=useState<Set<string>>(new Set());
 useEffect(()=>{
  if(!inviteSubTeamId)return;
  const choose=(event:PointerEvent)=>{const target=event.target as HTMLElement;if(target.closest('button,input'))return;const row=target.closest<HTMLElement>('[data-player-id]');if(!row)return;const player=players.find(item=>item.id===row.dataset.playerId);if(!player||!['current','waiting'].includes(player.status))return;event.preventDefault();event.stopImmediatePropagation();selectSubTarget(player)};
 ...
 const dragRef=useRef<{player:Player|null;row:HTMLElement|null;pointerId:number|null;preview:HTMLElement|null;timer:number|null;active:boolean;scrolling?:boolean;lastX:number;lastY:number;startX:number;startY:number;scrollY:number;frame:number|null}>({player:null,row:null,pointerId:null,preview:null,timer:null,active:false,scrolling:false,lastX:0,lastY:0,startX:0,startY:0,scrollY:0,frame:null});
 ...
 const allTeamsOccupied=teams.length>0&&teams.every(team=>team.members.length>0);
 const showWaitingPlaceholder=!hasOpenCourtTeam&&(waiting.length===0||allTeamsOccupied);
 const waitingPlaceholderStart=13+waiting.length*6;
 const reservedTeamNumbers=new Set(teams.map(team=>Number(team.name.match(/\d+/)?.[0])).filter(Number.isFinite));
 const placeholderTeamNumbers=new Map<string,number>();
 const teamNumberForPlaceholder=(key:string,preferred:number)=>{
  const existing=placeholderTeamNumbers.get(key);if(existing)return existing;
  let number=Math.max(1,preferred);while(reservedTeamNumbers.has(number))number+=1;
  reservedTeamNumbers.add(number);placeholderTeamNumbers.set(key,number);return number;
 };
 // dropRef is the synchronous source of truth while a pointer is moving.
 // Do not mirror rendered state back into it: React may commit an older
 // placeholder after a newer pointermove and make the target jump backwards.
 const candidateKey=(candidate:HTMLElement)=>candidate.dataset.kingTeamId??candidate.dataset.emptyKingTarget??null;
 ...
 const renderedDropPlacement=()=>{const emptyCard=document.querySelector<HTMLElement>('.king-empty-drop-target[data-empty-king-target]');const emptyTeamId=emptyCard?candidateKey(emptyCard):null;if(emptyTeamId)return{teamId:emptyTeamId,index:0};const placeholder=document.querySelector<HTMLElement>('.king-drop-placeholder');const card=placeholder?.closest<HTMLElement>('[data-king-team-id],[data-empty-king-target]');const teamId=card?candidateKey(card):null;if(placeholder&&card&&teamId){const rows=[...card.querySelectorAll<HTMLElement>('.king-player-row')];const index=rows.indexOf(placeholder);if(index>=0)return{teamId,index}}return null};
 const resolveTarget=(x:number,y:number)=>{const point=constrainQueueDragPoint(x,y);
  ...
  if(!card){const nearest=candidates.sort((a,b)=>distance(a)-distance(b))[0]??null;if(nearest&&distance(nearest)<=120)card=nearest;}
  ...
  if(!card&&dropRef.current){const current=candidates.find(candidate=>candidateKey(candidate)===dropRef.current?.teamId)??null;const rect=current?.getBoundingClientRect();if(current&&rect&&point.x>=rect.left-4&&point.x<=rect.right+4&&point.y>=rect.top-4&&point.y<=rect.bottom+4)card=current;}
  ...
  // Rows no longer animate underneath the pointer, so the fixed slot grid can
  // follow the pointer directly. This removes the sticky buffer at position 1
  // and makes cross-team drops use the visible placeholder exactly.
  const index=Math.max(0,Math.min(Math.floor(Math.max(0,point.y-contentTop)/rowHeight),memberCount));const current=dropRef.current;
 ...
 const startAutoScroll=()=>{if(dragRef.current.frame!==null)return;const tick=()=>{const state=dragRef.current;if(!state.active){state.frame=null;return;}const edge=Math.min(88,Math.max(64,window.innerHeight*.1));const root=document.scrollingElement??document.documentElement;const scrollTop=state.scrollY;const maxScroll=activeMobileDragMaxScroll;const canUp=scrollTop>1;const canDown=scrollTop<maxScroll-1;const direction=state.lastY<edge&&canUp?-1:state.lastY>window.innerHeight-edge&&canDown?1:0;if(direction){const distanceToEdge=direction<0?state.lastY:window.innerHeight-state.lastY;const strength=Math.max(.25,1-Math.max(0,distanceToEdge)/edge);const nextTop=Math.max(0,Math.min(maxScroll,scrollTop+direction*(6+20*strength)));scrollActiveMobileDrag(nextTop);state.scrollY=nextTop;if(state.preview)positionPlayerDragPreview(state.preview,state.lastX,state.lastY);resolveTarget(state.lastX,state.lastY)}state.frame=requestAnimationFrame(tick)};dragRef.current.frame=requestAnimationFrame(tick)};
 const activateDrag=(player:Player,start:number)=>{const state=dragRef.current;const row=state.row;if(!row)return;captureDragPickupViewport(row);state.player=player;state.timer=null;state.active=true;document.body.classList.add('king-drag-holding');setMobileAdminDragging(true,state.scrollY);document.addEventListener('touchmove',preventNativeTouchScroll,{passive:false});if(state.pointerId!==null)try{row.setPointerCapture(state.pointerId)}catch{}const preview=createPlayerDragPreview(player,[player],start);preview.classList.add('mobile-admin-drag-preview','king-player-drag-preview');document.body.appendChild(preview);state.preview=preview;positionPlayerDragPreview(preview,state.lastX,state.lastY);setDragging(player.id);navigator.vibrate?.(35);resolveTarget(state.lastX,state.lastY);startAutoScroll()};
 const beginDrag=(player:Player,event:ReactPointerEvent<HTMLElement>,start:number)=>{if(busy||(event.target as HTMLElement).closest('button,input'))return;const row=(event.target as HTMLElement).closest<HTMLElement>('[data-player-id]');if(!row)return;const state=dragRef.current;state.player=player;state.row=row;state.pointerId=event.pointerId;state.lastX=event.clientX;state.lastY=event.clientY;state.startX=event.clientX;state.startY=event.clientY;state.scrollY=window.scrollY;state.scrolling=false;if(event.pointerType==='mouse'){event.preventDefault();window.getSelection()?.removeAllRanges();activateDrag(player,start)}else{document.addEventListener('touchmove',preventNativeTouchScroll,{passive:false});state.timer=window.setTimeout(()=>{window.getSelection()?.removeAllRanges();activateDrag(player,start)},MOBILE_DRAG_HOLD_MS)}};
 useEffect(()=>{if(!operator)return;const cancelHoldOnScroll=(event:PointerEvent)=>{if(event.pointerType!=='touch')return;const state=dragRef.current;if(state.active||state.timer===null)return;if(Math.hypot(event.clientX-state.startX,event.clientY-state.startY)<=MOBILE_SCROLL_CANCEL_DISTANCE)return;clearTimeout(state.timer);state.timer=null;state.player=null;state.row=null;state.pointerId=null;document.removeEventListener('touchmove',preventNativeTouchScroll)};document.addEventListener('pointermove',cancelHoldOnScroll,{passive:true,capture:true});return()=>document.removeEventListener('pointermove',cancelHoldOnScroll,{capture:true})},[operator]);
 useEffect(()=>{if(!operator)return;const moveActiveTouchDrag=(event:PointerEvent)=>{if(event.pointerType!=='touch')return;const state=dragRef.current;if(!state.active)return;event.preventDefault();state.lastX=event.clientX;state.lastY=event.clientY;if(state.preview)positionPlayerDragPreview(state.preview,event.clientX,event.clientY);resolveTarget(event.clientX,event.clientY);preserveDragScrollAwayFromEdges(event.clientY,state.scrollY);startAutoScroll()};document.addEventListener('pointermove',moveActiveTouchDrag,{passive:false,capture:true});return()=>document.removeEventListener('pointermove',moveActiveTouchDrag,{capture:true})},[operator,teams]);
 useEffect(()=>{if(!operator)return;const preventSelection=()=>{if(document.body.classList.contains('king-drag-holding'))window.getSelection()?.removeAllRanges()};const updatePosition=(x:number,y:number,prevent?:()=>void)=>{const state=dragRef.current;state.lastX=x;state.lastY=y;if(!state.active){if(state.timer!==null&&Math.hypot(x-state.startX,y-state.startY)>24){clearTimeout(state.timer);state.timer=null;state.player=null;state.row=null;state.pointerId=null;document.body.classList.remove('king-drag-holding');document.removeEventListener('touchmove',preventNativeTouchScroll);}return;}prevent?.();if(state.preview)positionPlayerDragPreview(state.preview,x,y);resolveTarget(x,y);preserveDragScrollAwayFromEdges(y,state.scrollY);startAutoScroll()};const move=(event:PointerEvent)=>{if(event.pointerType==='touch')return;updatePosition(event.clientX,event.clientY,()=>event.preventDefault())};const touchMove=(event:TouchEvent)=>{const touch=event.touches[0];if(!touch)return;updatePosition(touch.clientX,touch.clientY,()=>event.preventDefault())};const finish=()=>{const state=dragRef.current;if(state.timer!==null)clearTimeout(state.timer);document.removeEventListener('touchmove',preventNativeTouchScroll);stopAutoScroll();const playerId=state.active?state.player?.id??null:null;
// dropRef is the synchronous source that also drives the visible placeholder.
// Keep the rendered marker as the first choice, then use that matching state if
// React has not committed its final drag-frame render before pointerup.
const target=state.active?(renderedDropPlacement()??dropRef.current):null;
state.preview?.remove();document.body.classList.remove('king-drag-holding');setMobileAdminDragging(false);dragRef.current={player:null,row:null,pointerId:null,preview:null,timer:null,active:false,lastX:0,lastY:0,startX:0,startY:0,scrollY:0,frame:null};dropRef.current=null;setDragging(null);setDrop(null);clearEmptyTarget();if(playerId&&target)void movePlayer(playerId,target.teamId,target.index)};const cancelPointer=(event:PointerEvent)=>{if(event.pointerType==='touch'&&dragRef.current.active)return;finish()};document.addEventListener('selectionchange',preventSelection);document.addEventListener('pointermove',move,{passive:false,capture:true});document.addEventListener('touchmove',touchMove,{passive:false,capture:true});document.addEventListener('pointerup',finish,{capture:true});document.addEventListener('touchend',finish,{capture:true});document.addEventListener('touchcancel',finish,{capture:true});document.addEventListener('pointercancel',cancelPointer,{capture:true});return()=>{document.removeEventListener('selectionchange',preventSelection);document.removeEventListener('pointermove',move,{capture:true});document.removeEventListener('touchmove',touchMove,{capture:true});document.removeEventListener('pointerup',finish,{capture:true});document.removeEventListener('touchend',finish,{capture:true});document.removeEventListener('touchcancel',finish,{capture:true});document.removeEventListener('pointercancel',cancelPointer,{capture:true});document.removeEventListener('touchmove',preventNativeTouchScroll);document.body.classList.remove('king-drag-holding');setMobileAdminDragging(false);stopAutoScroll()}},[operator,teams,busy]);
 const card=(team:KingTeam,start:number,side:number,showStreak:boolean)=>{const substitutes=teamSubstitutes.filter(sub=>sub.team_id===team.id);const ownTeam=Boolean(me&&(team.members.some(member=>member.id===me.id)||substitutes.some(substitute=>substitute.player_id===me.id)));const expanded=expandedSubs.has(team.id);const toggle=()=>setExpandedSubs(current=>{const next=new Set(current);if(next.has(team.id))next.delete(team.id);else next.add(team.id);return next});return <div className="team-with-subs" key={team.id}><KingTeamCard team={team} me={me} canJoin={!operator&&!playerJoinLocked&&(!me||!['current','waiting'].includes(me.status)||team.id!==me.team_id)&&team.members.length<6} busy={busy} joinTeam={joinTeam} start={start} side={side} showStreak={showStreak} operator={operator} admin={admin} host={host} adminSitOut={adminSitOut} adminLeave={adminLeave} permissions={permissions} dragging={dragging} drop={drop} beginDrag={beginDrag} now={Date.now()} fillIns={fillIns} players={players} allTeams={teams} fillIn={fillIn} unfill={unfill}/><TeamSubstituteRoster team={team} substitutes={substitutes} expanded={expanded} canManage={operator||ownTeam} operator={operator} busy={busy} toggle={toggle} invite={()=>inviteSub(team)} removeSub={removeSub}/></div>};
 return <div className={`king-board ${dragging?'king-dragging':''}`}>{courts.map((court,courtIndex)=>{const active=teams.filter(team=>team.status==='current'&&team.court_number===court.court_number).sort((a,b)=>(a.court_side??1)-(b.court_side??1));const ownCourt=active.some(team=>team.id===me?.team_id);const mode=court.team_mode??'rotation';const maxWins=court.team_max_wins;const firstTeam=courtIndex*2+1;const playing=active.reduce((total,team)=>total+team.members.length,0);const matchup=[active[0]?.name??`Team ${firstTeam}`,active[1]?.name??`Team ${firstTeam+1}`].join(' vs ');return <section className="king-court queue-card" key={court.court_number}><header><div className="king-court-title"><h2>{courts.length>1?`COURT ${court.court_number} - ${matchup}`:`CURRENT GAME - ${matchup}`}</h2><div className="court-team-rule">{operator?<><select aria-label={`Court ${court.court_number} format`} value={mode} onChange={event=>void setCourtRules(court.court_number,event.target.value as TeamCourtMode,maxWins)}><option value="rotation">2 on, 2 off</option><option value="king">King of the Court</option></select>{mode!=='rotation'&&<><span>with</span><select aria-label={`Court ${court.court_number} consecutive games`} value={court.team_max_wins??'unlimited'} onChange={event=>void setCourtRules(court.court_number,mode,event.target.value==='unlimited'?null:Number(event.target.value))}><option value="2">2</option><option value="3">3</option><option value="unlimited">Unlimited</option></select><span>consecutive games MAX</span></>}</>:mode!=='rotation'?<span className="king-rule-display"><b>KING OF THE COURT</b><small>{court.team_max_wins==null?'No game limit':`(${court.team_max_wins} CONSECUTIVE GAMES MAX)`}</small></span>:<span>2 ON, 2 OFF</span>}</div></div></header><div className="king-playing-count">{playing} playing</div>{courts.length>1&&operator&&<button className="court-next-button" disabled={busy||active.length!==2||Boolean(me?.restricted)} onClick={()=>nextGame(court.court_number)}>Next game (Court {court.court_number})</button>}<div className="king-current-teams">{active.map((team,index)=>card(team,index*6+1,firstTeam+index,mode!=='rotation'))}{active.length<2&&Array.from({length:2-active.length},(_,index)=>{const targetKey=`empty:current:${court.court_number}:${active.length+index+1}`;const side=teamNumberForPlaceholder(targetKey,firstTeam+active.length+index);
return <section data-empty-king-target={targetKey} className={`king-team-card queue-team-block empty-team-block ${dragging&&drop?.teamId===targetKey?'king-empty-drop-target':''}`} key={`empty-team-${index}`}>
<div className="team-block-label"><strong>Team {side}</strong>{!operator&&me&&<button className={`king-join-button ${playerJoinLocked?'join-action-locked':''}`} disabled={busy||playerJoinLocked} onClick={()=>joinEmptyTeam(side)}>Join +</button>}</div>{Array.from({length:6},(__,slot)=>dragging&&drop?.teamId===targetKey&&slot===0?<article className="player-row king-player-row king-drop-placeholder" aria-label="Player drop position" key="drop-placeholder"><span className="position">{(active.length+index)*6+1}</span></article>:<article className="player-row king-player-row king-empty-row" key={slot}><span className="position">{(active.length+index)*6+slot+1}</span><div className="player-name"><strong>Open spot</strong></div></article>)}</section>})}</div></section>})}<section className="king-waitlist queue-card"><header><h2>Waitlist</h2><span>{waiting.length?`${waiting.length} team${waiting.length===1?'':'s'} waiting`:'No one waiting'}</span></header><div className="king-waiting-list">{waiting.map((team,index)=>card(team,13+index*6,courts.length*2+index+1,false))}{showWaitingPlaceholder?
...
<div className="team-block-label"><strong>Team {teamNumberForPlaceholder('empty:waiting',courts.length*2+1)}</strong>{!operator&&me&&<button className={`king-join-button ${playerJoinLocked?'join-action-locked':''}`} disabled={busy||playerJoinLocked} onClick={()=>joinEmptyTeam(teamNumberForPlaceholder('empty:waiting',courts.length*2+1))}>Join +</button>}</div>{Array.from({length:6},(_,slot)=>dragging&&drop?.teamId==='empty:waiting'&&slot===0?<article className="player-row king-player-row king-drop-placeholder" aria-label="Player drop position" key="drop-placeholder"><span className="position">{waitingPlaceholderStart}</span></article>:<article className="player-row king-player-row king-empty-row" key={slot}><span className="position">{waitingPlaceholderStart+slot}</span><div className="player-name"><strong>Open spot</strong></div></article>)}</section>:waiting.length===0?<p className="empty">Teams will appear here after the active courts are filled.</p>:null}</div></section></div>
}
