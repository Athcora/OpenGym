...
- Restarted that verification safely: signed into the reset fixture in a fresh browser client, advanced Game 1 to Game 2, and queried it from a brand-new read-only Supabase SQL tab. Authoritative state is now Game 2, 12 current, 0 waiting, and 1 past game. A second freshly authenticated browser client is now ready, but the concurrent reverse-versus-next confirmation has not yet been repeated.
...
Continue the already-prepared guarded Next-versus-Reverse interleaving; do not restart it. Disposable fixture `qa-interleaving-20260917` is authoritatively at Game 2 with QA1–QA12 current, 0 waiting, and one reversible Game 1 record (verified from fresh read-only SQL query `0dea72a1-30f2-4fa2-b35b-4d4357765e3e`). Browser tab 7 is an authenticated admin showing Game 2. Browser tab 8 has just completed the second admin sign-in and has a `Signed in.` dialog; dismiss it, let the dashboard load, then use tab 7 to open Past Games → Reverse for Game 1 and leave its confirmation open, use tab 8 to open Next Game for the displayed Game 2 and leave its confirmation open, then confirm both concurrently. Immediately re-run the read-only SQL query (never the old destructive query `5883c47a-8b8b-490d-8a43-95c9f5cdfd57`) to verify exact game/current/waiting/past-game counts, and refresh a still-authenticated browser client. Only after that is authoritative should you proceed to guarded facility-switch interleaving using demonstrably shared Supabase auth storage. Preserve PHR and Ocean Air. If the UI harness cannot create shared-auth tabs, use an actual browser profile/session method rather than claiming the test ran. Then move into Run 3 action/permission inventory from the attached QA mission. For every confirmed defect, add a behavioral regression test, deploy it, browser-test it, run `node --test tests/*.test.mjs`, and update this file before stopping.
