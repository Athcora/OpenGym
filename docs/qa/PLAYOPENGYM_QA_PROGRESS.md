...
Continue Run 2 with the guarded Next-vs-Reverse interleaving; do not repeat the verified stale-confirmation races. Fixture `qa-concurrency-20260917-b` is no longer suitable because Team 1's rejoin timeout expired and its successful guarded Game 10 reversal intentionally kept those expired players gone, leaving Court 1 Next Game disabled. Create a fresh disposable QA-only facility or reset/reseed fixture A/B through the Supabase SQL editor with two full Court 1 teams, one active Next Game, a latest reversible history entry, and known QA-only administrator credentials. Use two isolated browser sessions: hold the latest Court 1 reverse confirmation in one and a stale Court 1 Next Game/Team winner flow in the other, confirm simultaneously, then inspect every player, team, game number, rejoin state, and Court 2 before and after refresh. Then test a facility-switch interleaving. Preserve PHR and Ocean Air. For every confirmed defect, add a behavioral regression test, deploy it, browser-test it, run `node --test tests/*.test.mjs`, and update this file before stopping.
