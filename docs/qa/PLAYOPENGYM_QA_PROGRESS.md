...
Do not repeat completed Regular Rejoin accept, explicit Leave, timeout, facility-scope migration, or the completed Group/Swap/Next Game/Reverse work. The Teams Mode (Rejoin) Device B prompt is **already prepared** on disposable `qa-multidevice-20260918`: `Ipad` is `rejoin`, belongs to the QA Rejoin Team, and has a pending Game 3 response expiring at `2026-09-19 19:21:10 UTC`. Ask the user only to open the existing QA OpenGym page on the physical iPad (refresh once only if no prompt appears) and press **Rejoin**. Then query the facility-scoped player/team/response rows, refresh the iPad page, and record the result. Next prepare a separate minimal Teams Rejoin timeout/leave fixture without touching PHR or Ocean Air and verify it in browser plus SQL. Then continue the remaining Run 3 host appointment/demotion, team substitute/fill-in, duplicate-device, and real-device geofence tests. Preserve PHR, Ocean Air, and the current Ipad session wherever possible.
