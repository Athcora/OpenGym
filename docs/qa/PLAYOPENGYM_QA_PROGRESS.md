...
Do not repeat the completed Regular/Teams Rejoin accept, explicit Leave, timeout, facility-scope migration, Group/Swap, Next Game/Reverse, or host appointment/demotion work. Continue Run 3 with **team substitutes and fill-ins** on disposable `qa-multidevice-20260918`. Prepare the fixture from Device A/admin without resetting it, then ask Device B only to join once as `Ipad` and accept/decline the genuine substitute invitation. Verify substitute visibility, cancellation/removal, one-game fill-in and return behavior in both the browser and facility-scoped database records. After that, complete genuine duplicate-device and real-device geofence validation; preserve PHR and Ocean Air.
