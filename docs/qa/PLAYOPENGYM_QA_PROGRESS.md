...
Continue Run 2 by redoing the guarded Next-versus-Reverse interleaving with authoritative verification; do not claim the prior displayed-browser result as a completed test. Fixture `qa-interleaving-20260917` was accidentally reset by the Supabase editor's stale destructive query warning and is now Game 1 with QA1–QA12 current. Create a brand-new SQL-editor query tab (do not reuse query `5883c47a-8b8b-490d-8a43-95c9f5cdfd57`), seed/reseed the fixture only once, advance Game 1 to Game 2 in the browser, then hold reverse Game 1 and Next Game 2 in separate authenticated admin clients. Confirm concurrently. Immediately query the fixture from a second, newly-created **read-only** SQL tab; verify game, current/waiting counts, and past-game count, then refresh a still-authenticated browser client. Only then proceed to guarded facility-switch interleaving using demonstrably shared Supabase auth storage. Preserve PHR and Ocean Air. If the UI harness cannot create shared-auth tabs, use an actual browser profile/session method rather than claiming the test ran. Then move into Run 3 action/permission inventory from the attached QA mission. For every confirmed defect, add a behavioral regression test, deploy it, browser-test it, run `node --test tests/*.test.mjs`, and update this file before stopping.
