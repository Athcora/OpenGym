...
The Run 3 action inventory started on disposable `qa-interleaving-20260917` in Regular mode. A real guest successfully joined at queue position 13, the in-app browser correctly treated a second tab in the same device/browser profile as that same player (no duplicate player was created), the player completed Sit Out and visibly received sit-out priority, and Leave removed the player and returned the session to the entry screen. This browser harness has a single shared browser/auth/device profile: guest, admin, and all tabs share the same authenticated user/device. Consequently it cannot create two independent player identities or simultaneously preserve a player and an admin/host session. That makes live browser verification of request accept/decline, swap acceptance, host promotion/demotion behavior, stale host sessions, and multi-device duplicate protection technically impossible here without a second isolated browser profile or a real second device. Do not fabricate those outcomes from source tests or SQL seeding.

## NEXT SESSION — START HERE

Run 3 cannot honestly be marked complete until the remaining multi-identity inventory is browser-tested. Continue only when a second independent browser profile/device is available. Use one disposable QA facility in Regular mode and a second in Rejoin mode. Keep the current first-device results, then verify: group request accept/decline and leave/remove; swap request accept/decline; player Next Game; rejoin; appoint first and second host, reject a third host, host actions, demotion, host leave, and stale host action rejection; duplicate-device enforcement from a genuinely separate device; and the corresponding desktop/mobile/reconnect/database checks. Then complete the remaining admin move/group/substitute/fill-in/undo-redo/restriction and geofence scenarios. Preserve PHR and Ocean Air.
