...
5. Genuine device duplicate protection: Device/Profile A joins. Device/Profile B attempts to join while using the same persisted device identifier only if the product test setup can deliberately reproduce that identifier; otherwise verify normal separate devices can each join once. Confirm no duplicate active row exists for a shared device identifier and that a rejoin after leave is allowed.
6. Real-device geofence: on a configured disposable facility, test permission denied/unavailable, a known inside coordinate, outside-radius removal, and return-inside restoration. Confirm no player location columns/records are written; only facility configuration remains.
