...
Do not repeat completed Regular Rejoin accept, explicit Leave, timeout, facility-scope migration, or the completed Group/Swap/Next Game/Reverse work. Continue the unfinished **Teams Mode (Rejoin)** Device B validation on disposable `qa-multidevice-20260918` without resetting or signing out `Ipad`: prepare a minimal Teams Rejoin prompt for the existing Ipad player yourself, then ask the user only to press **Rejoin** on the physical iPad. Verify the player/team state and facility-scoped response row after refresh. Next prepare a separate minimal Teams Rejoin timeout/leave fixture without touching PHR or Ocean Air and verify it in browser plus SQL. Then continue the remaining Run 3 host appointment/demotion, team substitute/fill-in, duplicate-device, and real-device geofence tests. Preserve PHR, Ocean Air, and the current Ipad session wherever possible.
