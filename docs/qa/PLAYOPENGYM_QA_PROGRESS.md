...
Group and player-to-player Swap are now complete; do not redo them. Continue with the **Rejoin** flows in both `rejoin` and `teams_rejoin`, reusing Device B (`Ipad`) on `qa-multidevice-20260918` if feasible. Prepare all fixture/operator steps yourself. Ask the iPad only to accept the actual rejoin prompt, explicitly leave it, or allow/confirm timeout when each action genuinely needs its independent session. Verify each accept/leave/timeout/Accept All branch in the browser after refresh and via facility-scoped rejoin/player rows. Then continue the remaining host, team substitute/fill-in, duplicate-device, and real-device geofence scenarios; do not repeat the completed P0 Next Game/Reverse/concurrency/ACL or automatic Run 3 inventory. Preserve PHR and Ocean Air.
