...
Only run the seven second-device scenarios listed immediately above. Start with two isolated real browser profiles/devices on a new disposable Regular fixture for Group, Swap, and substitutes/fill-ins, then use a separate disposable Rejoin fixture for rejoin/host/geofence checks. Do not repeat the completed Next Game, Reverse, concurrency, ACL, admin action, mode/court, or single-profile player checks. Preserve PHR and Ocean Air. After each scenario, refresh both clients and record the facility-scoped database result here.
