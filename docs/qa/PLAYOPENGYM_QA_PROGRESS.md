...
Continue Run 2 with the guarded Next-vs-Reverse interleaving; do not repeat the verified stale-confirmation races. The live fixture is `qa-concurrency-20260917-b` at version 460, currently in Teams Mode (Rejoin), with Court 1 on Game 11 after Team 1 won Game 10 and exactly six Team 1 rejoin requests/timers pending. Browser tabs 20 and 23 are isolated QA administrator sessions. First open Past games in tab 20, identify the latest Court 1 Game 10 reversal card, and have one tab confirm Reverse while the other uses a stale Court 1 Next Game/Team 1 winner flow; inspect the final court, team, rejoin, and player state to ensure exactly one guarded transition wins and no player duplicates or disappears. If the Past games drawer fails to open again, inspect `openPastGames()` / `ensureFacilityContext()` and reproduce in a fresh QA-only session before modifying code. Then test a facility-switch interleaving. Preserve PHR and Ocean Air. For every confirmed defect, add a behavioral regression test, deploy it, browser-test it, run `node --test tests/*.test.mjs`, and update this file before stopping.
