...
assert.match(app,/const modeViewportRef=useRef<\{mode:Config\['mode'\];scrollY:number;layoutRevision:number\}\|null>\(null\)/);
assert.match(app,/modeTransitionInProgress\?'mode-transitioning':''/);
assert.match(app,/function settleModeViewport\(pending:\{scrollY:number;layoutRevision:number\},layoutRevision:number\)/);
assert.match(app,/if\(!settleModeViewport\(pending,modeLayoutRevision\)\)return/);
assert.match(app,/if\(!await rpc\('set_open_gym_mode',\{p_mode:mode\},false\)&&modeViewportRef\.current===pending\)/);
assert.match(advancedCss,/\.queue-page\.mode-transitioning\{overflow-anchor:none\}/);
