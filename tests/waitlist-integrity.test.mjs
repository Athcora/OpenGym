...
assert.match(app,/const modeViewportRef=useRef<\{mode:Config\['mode'\];scrollY:number\}\|null>\(null\)/);
assert.match(app,/modeTransitionInProgress\?'mode-transitioning':''/);
assert.match(app,/window\.scrollTo\(\{top:Math\.min\(pending\.scrollY,maxScroll\),left:0,behavior:'instant'\}\)/);
